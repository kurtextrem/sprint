...
;(function() {
	...
	var addPx = (function() {
		...
		return function addPx(cssProperty, value) {
			...
			var stringValue = isString(value) ? value : value.toString()
	...
	var getSetDimension = function(obj, prop, value) {
		...
		var stringValue = isFunc ? '' : addPx(prop, value)
	...
	var manipulateClass = function(method, className, bool) {
		...
		return this.each(function(i, el) {
			...
			for (var j = 0; j < classNamesLen; ++j) {
				...
				bool === undefined ? el.classList[method](name) : el.classList.toggle(name, bool)
			}
		})
	}
	...
	var removeEvent = (function() {
		var isHandlerShared = function(el, event, registeredHandler) {
			var similarEventsHandlers = Object.keys(getEvents(el))
	...
	var sanitize = function(arr, flattenObjects, requireDomNodes) {
		...
		while (i--) {
			...
			if (
				...
				(requireDomNodes && (isString(arr[i]) || isNumber(arr[i])))
			) {
				var sanitized = []
				...
				for (var j = 0; j < arrLen; ++j) {
					...
					var type = typeof el
					if (requireDomNodes && (type === 'string' || type === 'number')) {
	...
	var scroll = (function() {
		...
		return function(sprintObj, method, value) {
			...
			if (!scrollRoot) {
				...
				scrollRoot =
					updatedScrollPos > initialScrollPos
	...
	var wrap = (function() {
		var callback = function(wrappingElement, variant) {
			var wrap = Sprint(wrappingElement)
				.clone(true)
				.get(0)
	...
	Init.prototype = {
		...
		attr: function(name, value) {
			...
			if (type === 'string' || type === 'number' || isFunc) {
		...
		css: function(property, value) {
			...
			var isStr = valueType === 'string'

			// set

			if (isStr || valueType === 'number') {
				var isRelativeValue = isStr && /=/.test(value)
		...
		filter: function(selector) {
			var isFunc = isFunction(selector)
			var self = this
		...
		on: function(events, selector, handler, options) {
			if (typeof selector !== 'string') {
				handler = selector
				selector = null
			}
			...
			if (handler) {
				var useSelector = selector !== null && selector !== undefined
				if (useSelector) {
				...
				return this.each(function() {
					...
					eventsArr.forEach(function(event) {
						var events = getEvents(this)[event]
						if (!events) {
							events = []
						}
						...
						this.addEventListener(getEventFromNamespace(event), handler, options)
					}, this)
				})
			}
	...
	for (var i = 0; i < eventShortcuts.length; ++i) {
		var e = eventShortcuts[i]
		Init.prototype[e] = function(fn) {
			return this.on(e, fn)
		}
	}
